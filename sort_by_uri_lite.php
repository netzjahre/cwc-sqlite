...
if ($_GET[action]=="dump" && $_GET[id]<$number_of_sites) {
	...
	echo "<tr style='background-color:#a2a5a7;text-align: left;'>
		...
		$db->exec("CREATE INDEX IF NOT EXISTS idx_date_host_uri_ip_useragent_referer ON $tablename[$sid] (date(timestamp), http_host, request_uri, remote_addr, http_user_agent, http_referer)");
