...
//export entries///////////////////////////////////////////
		if(isset ($_POST[cbox]) && is_array($_POST[cbox]))
			{
			$cbox = $_POST[cbox];
			$csvfile = fopen('temp_exp.csv', 'w+');
			$db = new PDO("sqlite:$dbname");
			$db->exec("PRAGMA journal_mode = TRUNCATE;");
				foreach($cbox as $key => $value)
					{
					$result = $db->query("SELECT * FROM $tablename[$sid] WHERE id = '$key'");
					if ($row = $result->fetch()) {
						do 
							{
							$numrows += 1;
			...
			echo "</div>";
			echo "<div class='flex-container'><div>.</div></div>";
			echo "<div class='flex-container'>.</div></div>";
			echo "</div>";
?>
</body>
</html>
